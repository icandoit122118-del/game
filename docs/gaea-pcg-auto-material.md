...
툴/에셋 조합 추천과 마켓플레이스 구매 판단 기준은 6장에 정리했습니다.
...
### 6-1. 1순위 — 순정 조합 (대부분 여기서 시작하면 됩니다)

| 역할 | 선택 | 비고 |
|---|---|---|
| 지형 생성 | **Gaea 2.x** | 침식/퇴적 마스크 품질이 이 파이프라인의 전제 |
| 엔진 | **UE 5.5 이상** (최소 5.4) | PCG가 5.4에서 실용화, 런타임 생성은 5.5+에서 안정 |
| 지형 머티리얼 | **직접 제작 오토 머티리얼** (3장) | 노드 20개 남짓. 파라미터를 내가 아는 게 가장 큼 |
| 에셋 배치 | **순정 PCG** (4장) | 대체 불가. 배치 규칙은 프로젝트 고유 자산 |
| 나무/바위/서페이스 | **Fab (Megascans 계열)** | 라이선스 상태는 아래 6-4 참고 |
| 렌더링 보조 | Nanite + RVT + World Partition | 대형 맵이면 셋 다 사실상 필수 |

추가 비용: **Gaea 라이선스 외 0원.** 유료 에셋 없이 완결됩니다.

### 6-2. 시간이 급하다면 — 머티리얼만 구매

| 역할 | 선택 |
|---|---|
| 지형 머티리얼 | **Landscape Auto Material 계열 구매** |
| 에셋 배치 | 순정 PCG (직접) |

머티리얼은 "룩만 나오면 되는" 영역이라 사는 게 시간을 아껴줍니다.
반면 배치는 레벨 디자인 규칙 그 자체라 남의 것을 그대로 쓰기 어렵습니다.
**살 거라면 머티리얼을 사고 배치는 직접 만드세요.** 반대 조합은 거의 항상 손해입니다.

구매하더라도 경사/고도 판정 원리는 3-2, 3-3 과 동일하므로,
파라미터를 어디서 찾아 고칠지 알고 사는 것과 모르고 사는 것은 결과가 완전히 다릅니다.

### 6-3. 피해야 할 조합

- **오토 머티리얼과 폴리지 스캐터 에셋을 서로 다른 벤더에서 구매**
  → 두 에셋이 각자의 내부 마스크로 판정하므로 **마스크를 공유할 수 없습니다.**
  이 문서 전체의 전제(“머티리얼과 배치가 같은 마스크·같은 임계값을 쓴다”)가 깨져서,
  바위 텍스처 위에 잔디가 자라는 상태를 손으로 계속 보정하게 됩니다.
- **Gaea 마스크를 안 쓰고 UE 안에서 노이즈로 배치**
  → Gaea를 쓰는 의미가 사라집니다. 침식 골짜기와 퇴적 평지를 따라가는 배치가 핵심입니다.
- **Landscape Grass Type + PCG 혼용** (같은 종류의 식생에 대해)
  → 두 시스템이 같은 영역에 겹쳐 뿌립니다. 풀은 Grass Type, 나무·바위는 PCG 처럼
  **레이어를 명확히 나누거나**, 아예 PCG로 통일하세요.

### 6-4. 라이선스 확인이 필요한 부분

아래 두 가지는 시점에 따라 조건이 바뀌므로 **작업 전 공식 페이지에서 직접 확인하세요.**

- **Gaea 무료(Community) 티어의 빌드 해상도 상한** — 이 문서가 권장하는 2017 / 4033 해상도를
  구우려면 상위 티어가 필요할 수 있습니다. 1-2절의 해상도 계획을 세우기 전에 확인이 필요합니다.
- **Fab 의 Megascans 에셋 라이선스** — 과거 전량 무료 클레임이 가능하던 시기가 있었고 그 이후
  조건이 변경되었습니다. 현재 무료 범위와 상업적 사용 조건은 Fab에서 확인해야 합니다.

### 6-5. 규모별 요약

| 프로젝트 | 권장 |
|---|---|
| 개인 / 포트폴리오 / 프로토타입 | 6-1 순정 조합. 유료 에셋 불필요 |
| 소규모 팀, 일정 촉박 | 6-2 (머티리얼 구매 + PCG 직접) |
| 장기 유지·확장하는 프로덕션 | 6-1 순정 조합. 남의 머티리얼 그래프는 디버깅 비용이 계속 누적됩니다 |
