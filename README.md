# game — 스카이림 모딩 툴을 만들며 배우는 프로그래밍 기초
...
| 4 | `lessons/04_lists_and_loops.py` | 리스트, for, enumerate, while | 로드 오더 목록 |
| 5 | `lessons/05_functions.py` | def, 매개변수, return | 기능을 함수로 나누기 |
| 6 | `lessons/06_read_plugins_txt.py` | 파일 읽기/쓰기, pathlib | plugins.txt 읽기 |
| 7 | `lessons/07_dict_masters.py` | 딕셔너리, 중첩 반복 | 마스터 누락/순서 검사 |
| 8 | `lessons/08_read_esp_header.py` | 바이너리 파일, struct, 비트 플래그 | .esp 헤더에서 마스터 목록 읽기 |
